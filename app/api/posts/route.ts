...
import { hashDeletePassword } from "@/lib/password";
...
export async function POST(request:NextRequest){
  ...
  const bookSlug=clean(body.bookSlug),nickname=clean(body.nickname),title=clean(body.title),
        content=clean(body.content),deletePassword=clean(body.deletePassword),website=clean(body.website);
  ...
  if(deletePassword.length<4||deletePassword.length>32)return NextResponse.json({error:"삭제 비밀번호는 4~32자로 입력하세요."},{status:400});
  ...
  const {error}=await supabase.from("posts").insert({
    book_slug:bookSlug,nickname,title,content,delete_password_hash:hashDeletePassword(deletePassword),status:"visible"
  });
  if(error)return NextResponse.json({error:"글을 등록하지 못했습니다."},{status:500});
